...
To reflect the expectation of a "medium" effect size, a researcher can, for example, choose the default prior distribution [i.e., a Cauchy distribution with scale parameter *r* = $\sqrt2/2$\; @morey2011; @schnuerch2020; @schonbrodt2017]. Alternatively, a researcher can rely on an informed prior distribution that incorporates prior knowledge about the effect size, gathered from expert knowledge, previous work, or theoretical considerations [@stefan2020]. In addition, the researcher could (for example, on the basis of simulations) choose $BF_{+0}$ = 3 as a decision threshold [commonly referred to as "*moderate evidence*"\; @schonbrodt2017]. This decision threshold _t_* implies that the researcher is content to stop sampling once the data is 3 times more likely under $H_+$ than under $H_0$, or vice versa. Figure 2A (2B) provides a visual example of a situation in which the Bayes Factor indicates that the data is 3 times more likely under $H_+$ ($H_0$).
...
3. For the SBF procedure, $\delta$ = 0 and the decision was made to support the null hypothesis ($BF_{0+} \ge$ _t_*)
...
The parameters for the Sequential Bayes Factor procedure were chosen to allow for maximum comparability with the frequentist sequential procedures, which correspond to a one-tailed hypothesis test powered to detect a hypothesized population effect size of $\delta$ = 0.5 with $\alpha$ = .05 and $k_{max} = 3$ looks. To calculate the Bayes Factor, I used an informed prior (a normal distribution centered at the hypothesized effect size, $\delta = 0.5$). To incorporate information about the sidedness of the hypothesized effect, the prior distribution was truncated at $\delta = 0$ to include only positive values. Thus, like in the one-sided *t*-tests described above, we compare $H_0:~\delta = 0$ to $H_+:~\delta > 0$.  

For the SBF procedure, I ran simulations to choose an optimal sample size (paired to the other procedures under study, with $k_{max}$ = 3 equally spaced looks) and Bayes Factor threshold, such that the overall error rate would approach that of the frequentist procedures (i.e., a false negative rate of 20% and a false positive rate of 5%). In the main paper, I report the results of a max-SBF procedure with the Bayes Factor threshold set to 3 for $H_+$ and 1/3 for $H_0$, the sample size set to $N_{\text{Bayes}}$ = 25 per group per look with $k_{max}$ = 3 looks, and a normal prior distribution centered at $\delta = 0.5$ with $\sigma^2 = 0.3$. In the supplemental materials at [link], results can be found for simulations that vary additional parameters for the max-SBF procedure. Of note, as discussed above, the Sequential Bayes Factor can be implemented much more flexibly (i.e., with as many looks as desired, without a maximum sample size). However, to make the sequential procedures as comparable as possible and to account for practical considerations that researchers may have, each procedure is each implemented with $k_{max}$ = 3 looks. 
...
2. Run a one-sided, two-sample Bayes Factor *t* test on the mean difference between the two samples
  a. if $BF_{+0} \ge 3$: decide to reject $H_0$ (support $H_1$) and end the experiment
  b. if $BF_{+0} \le 1/3$: decide to reject $H_1$ (support $H_0$) and end the experiment
...
The error rates involved in the procedures under discussion are shown in Table 2A, Table 2B, and Figure 4. Table 2A describes the rates of True Negative, Inconclusive, and False Positive results when the true population effect size $\delta$ = 0; Table 2B describes the rates of True Positive, Inconclusive, and False Negative results when the true population effect size $\delta$ = 0.50. As can be read from the tables, the frequentist procedures all perform as expected: 4-5% of simulations rejected the null hypothesis when $\delta$ = 0 (i.e., a type I error rate of 4-5%; see the False Positive column Table 2A) and 80-81% of simulations rejected the null hypothesis when $\delta$ = 0.5 (i.e., a type II error rate of 19-20%; see the True Positive column of Table 2B).
...
<!-- Re. O'Brien-Fleming: discuss the very conservative stopping boundary at early analysis, which also makes the decision rule similar to the fixed sample test if the last stage is reached -- "features which have turned out to be very appealing to practitioners"[@jennison2000: p.25] -->
...
<!-- "From a practical perspective, there are many hypothesis testing contexts where it is not feasible to implement a SPRT. For instance, a SPRT cannot be applied when data are not collected sequentially. Similarly, it cannot be applied when it is not possible to perform the evaluation as soon as participants are treated... To address these challenges, group sequential designs have been developed to allow for the evaluation of patient outcomes only after groups of patients have been observed or at scheduled interim analysis times" [@pramanik2021: p. 4-5] -->

<!-- "Continuous data monitoring can be a serious practical burden, and the introduction of group sequential tests has led to much wider use of sequential methods. (...) Not only are group sequential tests convenient to conduct, they also provide ample opportunity for early stopping and can achieve most of the benefits of fully sequential tests in terms of lower expected sample sizes and shorter average study lengths." [@jennison2000: p. 38] -->

<!-- # To add -->

<!-- Add in discussion/limitation section: We do not discuss adaptive designs, which extend GS designs and allow for even more flexibility while controlling error rates.  -->
<!-- Discuss the power of the individual segments. Practical advice (can lead to guidance on choosing alpha strong; make sure that the alpha strong you set does not exclude the effect size you are interested in. Write reveal.effectsize function): You have to make sure that the individual segments are each powered to at least 50 percent to detect the hypothesized effect size: otherwise, the critical effect size will be higher than the hypothesized effect size, and you will thus systematically discard the effect sizes that you intended to detect! Show that with the example used, the first two independent segments are powered to ~30%. In all but the last segment, the critical effect size (the effect size you need to observe to claim significance) is a factor larger than the actual population effect that you powered your test for. It is thus impossible to get an unbiased estimate out of anything but the last segment.  When you obtain an effect size close to the true effect, the ISP procedure forces you to continue to the last segment, and *throw out that data*. (You can even graph this exaggeration ratio: create a table that shows the exact critical effect size for all possible combinations, and then you can just read off that plot that you're setting yourself up for overestimation). -->
<!-- Discuss conditional bias (bias in effect size estimates conditional on stopping early) and unconditional bias (bias in effect size estimates across all studies). All other procedures have low unconditional bias, when you take the late termination studies into account, because they do cumulative analyses — the late termination studies are able to compensate for the bias in the early stage terminations. However, because the independent segments procedure is independent, each individual segment is biased and this is never accounted or compensated for. Thus, the ISP is unique among sequential procedures in that it not only has strong conditional bias (like most sequential procedures): it also has strong unconditional bias. [In this paper, I focused on two properties: conditional bias (bias in effect size estimates conditional on stopping early) and unconditional bias (bias in effect size estimates across all studies). I show that the independent segments procedure is unique among sequential procedures in that it not only has strong conditional bias (like all sequential procedures); it also has strong unconditional bias.] Emphasize that this is also what we see in Figure 3B: we only reject when d > 0.57. (This is not the case for, e.g., Pocock)   -->
<!-- In the language of decision theory, the ISP is inadmissible: its total squared error risk exceeds that of other hypothesis testing procedures no matter what the true effect size may be.  -->
<!-- Change writing on Bayes True Negative. Because I switched to an oracle prior setup, true negative rates actually look great -->
<!-- Also refer to MSE as reducible error  -->
<!-- Maybe add the figure of bimodality for all ISP parameters? Look through previous commits for this. -->
<!-- Emphasize that there is no bias correction available for the ISP, while there is for the SBF and GS. Also emphasize that the ISP is the only procedure that is *not* median unbiased. Check if Lakens' comment makes this point? -->
<!-- Incorporate points from three new Psych Methods papers (Lakens, Sneurch/Erdfelder, Ulrich/Miller) -->
<!-- (To make the point that the setup of the simulations is more practical and realistic than previous ones), Show some data on what scientists actually do, e.g., how often they look at their data when they use the Sequential Bayes Factor. Here's some examples on how SBFs are used in practice https://github.com/nicebread/BFDA/blob/master/BFDA_examples.md  -->
...
| **Acceptance of the null** | Possible | Impossible | Possible |
...
*Note.* The table shows the median effect size estimate across simulations. The effect size measure is Cohen's d. 
