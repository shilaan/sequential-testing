...
*Note.* Average sample size required for five hypothesis testing procedures across a range of unexpected effect sizes. All frequentist procedures were powered to detect an effect of $\delta$ = 0.5. For the Sequential Bayes Factor procedure, simulations were run to choose an optimal sample size, decision threshold, and prior distribution, with the goal of approach similar error rates as the frequentist procedures.   
...
*Note.* Pocock-like group-sequential design for a one-tailed, two-sample *t* test with $\alpha$ = .05, $\beta$ = .20 and $k_{max}$ = 3 equally spaced looks. Critical values are determined on the basis of Pocock-like $\alpha$- and $\beta$-spending functions with bindings futility bounds.
